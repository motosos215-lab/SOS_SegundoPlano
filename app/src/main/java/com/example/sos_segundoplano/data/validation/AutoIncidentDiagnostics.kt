...
object AutoIncidentDiagnostics {
    ...
    fun storageException(stage: String, failure: Throwable) {
        debug(
            "event=auto_incident_storage_exception " +
                "stage=${safeType(stage)} exception=${safeType(failure::class.java.simpleName)}"
        )
    }

    fun validationCandidate(kind: String) {
        debug("event=auto_incident_validation_candidate kind=${safeType(kind)}")
    }
    ...
    fun tripFinalizationClaim(result: String) = debug("event=auto_trip_finalization_claim result=${safeType(result)}")
    fun tripFinalizationStarted() = debug("event=auto_trip_finalization_started")
    fun tripLocalReconcileResult(result: String) = debug("event=auto_trip_local_reconcile_result result=${safeType(result)}")
    fun tripFinalizationComplete(result: String) = debug("event=auto_trip_finalization_complete result=${safeType(result)}")
    fun tripFinalizationRetryScheduled(result: String) = debug("event=auto_trip_finalization_retry_scheduled result=${safeType(result)}")
