...
data class RemoteIncidentLink(
    ...
    /** Rider that owns this durable request. Null exists only for legacy/test records. */
...
class SharedPreferencesRemoteIncidentLinkStore(
    ...
    override fun read(clientIncidentId: String): RemoteIncidentLink? = synchronized(lock) {
        ...
        RemoteIncidentLink(
            ...
            ownerUserId = preferences.getString("$keyPrefix.$KEY_OWNER_USER_ID", null).normalized()
        )
    }
    ...
    override fun save(link: RemoteIncidentLink): Boolean = synchronized(lock) {
        ...
        val ownerUserId = link.ownerUserId.normalized()
        ...
        val editor = preferences.edit()
            ...
            .putNullableString("$keyPrefix.$KEY_OWNER_USER_ID", ownerUserId)
