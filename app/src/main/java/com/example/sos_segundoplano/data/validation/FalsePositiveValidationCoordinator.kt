        ...
        events.forEach { event ->
            when (event) {
                ...
                is PendingOfflineEvent.IncidentBundleEvent -> {
                    val eventPreparedForPersistence = if (
                        event.incident.isAutomaticSosCause() &&
                        automaticSosAlertCreator !== NoOpAutomaticSosAlertCreator
                    ) {
                        // A persistence/remote retry must not recapture a different trip or GPS point when the
                        // original emergency context is already complete. Only resolve fields that are missing.
                        val incidentWithRemoteTrip = if (event.incident.remoteTripId.isNullOrBlank()) {
                            automaticSosAlertCreator.resolveRemoteTrip(event.incident)
                        } else {
                        ...
                        if (incidentWithRemoteTrip.remoteCreationStatus !is IncidentRemoteCreationStatus.Pending) {
                            publishRemoteFailureForRetry(
                                event.copy(incident = incidentWithRemoteTrip),
                                incidentWithRemoteTrip
                            )
                            return@forEach
                        }
                        val incidentWithLocation = if (incidentWithRemoteTrip.hasValidAutomaticSosLocation()) {
                            incidentWithRemoteTrip.copy(remoteCreationStatus = IncidentRemoteCreationStatus.Pending)
                        } else {
                            automaticSosAlertCreator.captureLocation(incidentWithRemoteTrip)
                        }
                        if (incidentWithLocation.remoteCreationStatus !is IncidentRemoteCreationStatus.Pending) {
                            publishRemoteFailureForRetry(
                                event.copy(incident = incidentWithLocation),
                                incidentWithLocation
                            )
                            return@forEach
                        }
                        event.copy(incident = incidentWithLocation).also { prepared ->
                            // The very first durable bundle already contains the remote trip and GPS context.
                            // Retries reuse this exact object and therefore the same client IDs/session identity.
    ...
    private suspend fun publishRemoteFailureForRetry(
        ...
        mutex.withLock {
            if (attempt != null) {
                ...
                publish(FalsePositiveValidationState.Error(event.metadata, "RemoteIncidentCreationFailed"))
                AutoIncidentDiagnostics.terminalState("error", "remote_incident_retry_exhausted")
            }
        }
    }
